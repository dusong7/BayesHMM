...
double Comp_Normal(vector<double> x, vector<double> mu,
    ...
    void Run_S_only(int step, int burnin, int thin);
...
void HMM::Run_S_only(int step, int burnin, int thin){
	Upd_Counts();
...
public:
    ...
    Posterior_Mode(HMM hmm){
   	 	int mode_loc=max_loc(hmm.loglik_trace);
    	mu=hmm.mu_trace[mode_loc];
    	sigma=hmm.sigma_trace[mode_loc];
    	q=hmm.q_trace[mode_loc];
    	pi0=hmm.pi0_trace[mode_loc];
    }
};

void UpdateHMM_w_Est(HMM& hmm, Posterior_Mode mode){
	int K=hmm.K;
	int p=hmm.p;
	...
	{
		for (int j = 0; j < p; ++j){
			hmm.mu[k][j]=mode.mu[k*p+j];
			hmm.sigma[k][j]=mode.sigma[k*p+j];
		}

		for (int j = 0; j < K; ++j){
			hmm.q[k][j]=mode.q[k*K+j];
		}
	}
}