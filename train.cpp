...
extern "C" void train_HMM(
  ...
  */
  ...
  HMM hmm_train(*K, X_full);
  hmm_train.Burnin(*burnin);
  hmm_train.Run(*step,*thin);

  Posterior_Mode hmm_est(hmm_train);

	copy(hmm_est.mu, best_mu);
	copy(hmm_est.sigma, best_sigma);
	copy(hmm_est.q, best_q);
	copy(hmm_est.pi0, best_pi0);


	copy(flatten(hmm_train.mu_trace),trace_mu);
	copy(flatten(hmm_train.sigma_trace),trace_sigma);
	copy(flatten(hmm_train.q_trace),trace_q);
	copy(flatten(hmm_train.pi0_trace),trace_pi0);

	copy(hmm_train.loglik_trace,trace_loglik);
